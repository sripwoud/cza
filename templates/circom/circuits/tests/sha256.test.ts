...
describe("sha256", () => {
  ...
  it("should fail on fake witness", async () => {
    ...
    const badWitness = await circuit.editWitness(witness, {
      "main.sha256.sha256compression[0].sigmaPlus[38].sigma0.out[1]":
        BigInt(1234),
      "main.sha256.sha256compression[0].sigmaPlus[38].sigma0.out[2]":
        BigInt(1234),
      "main.sha256.sha256compression[0].sigmaPlus[38].sigma0.out[3]":
        BigInt(1234),
      "main.sha256.sha256compression[0].sigmaPlus[38].sigma0.out[4]":
        BigInt(1234),
      "main.sha256.sha256compression[0].sigmaPlus[38].sigma0.out[5]":
        BigInt(1234),
    });
